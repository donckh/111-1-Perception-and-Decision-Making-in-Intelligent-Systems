...
def display_inlier_outlier(cloud, ind):
    ...
    outlier_cloud = o3d.open3d.geometry.select_down_sample(cloud, ind, invert=True)  # invert利用索引反选
...
def custom_down_sample(pcd, voxel_size):
    arr = np.asarray(pcd.points)  # Convert Open3D.o3d.geometry.PointCloud coordinate(x,y,z) to numpy array
    color = np.asarray(pcd.colors)  # Convert Open3D.o3d.geometry.PointCloud color(RGB) to numpy array

    arr_max = np.max(arr, axis=0)  # get the max value of coordinate(x,y,z)
    arr_min = np.min(arr, axis=0)  # get the min value of coordinate(x,y,z)
    # print("arr_max: {}, arr_min: {}".format(arr_max, arr_min))
    # split coordinate(X,Y,Z) from min to max according to voxel_size
    X_segments = np.arange(arr_min[0], arr_max[0] + 1, voxel_size)
    Y_segments = np.arange(arr_min[1], arr_max[1] + 1, voxel_size)
    Z_segments = np.arange(arr_min[2], arr_max[2] + 1, voxel_size)
    edges = [X_segments, Y_segments, Z_segments]  # combine the x,y,z segments values

    # classify arr array to class[x, y, z], b:grid value
    coords = np.vstack([np.digitize(arr.T[i], b, right=True) for i, b in enumerate(edges)]).T
    # print(coords.shape)
    grid = np.unique(coords, axis=0)
    # print("grid: \n", grid.shape)

    # create down sample arr for coordinate and colors
    arr_down = np.array([])
    arr_down_color = np.array([])
    for i in grid:
        idx = np.all(coords == i, axis=1)  # find the idx of arr which match i class
        idx = np.where(idx)  # convert mask to idx for reduce operation time
        # get the mean value of coordinate from the same grid
        mean = np.mean(arr[idx], axis=0)  # idx
        # print("mean: \n", A[np.all(coords == i, axis=1)])
        # print(mean)
        arr_down = np.concatenate((arr_down, mean))  # append mean value to down sample arr

        # get the most frequent color appears in the grid to represent the color for this grid
        mean_color, counts = np.unique(color[idx], axis=0, return_counts=True)
        # print("mean_color: {}, counts: {}, argmax: {}".format(mean_color, counts, counts.argmax()))
        arr_down_color = np.concatenate((arr_down_color, mean_color[counts.argmax()]))  # append color to down sample arr

    arr_down = arr_down.reshape(-1, 3)  # reshape as xyz shape
    arr_down_color = arr_down_color.reshape(-1, 3)  # reshape as rgb shape
    # print(arr_down.shape)
    # print(arr_down_color.shape)
    pcd_result = o3d.geometry.PointCloud()  # create a point cloud
    pcd_result.points = o3d.Vector3dVector(arr_down)  # assign down sample arr to points
    pcd_result.colors = o3d.Vector3dVector(arr_down_color)  # assign down sample arr to colors
    return pcd_result  # return point cloud


def preprocess_point_cloud(pcd, voxel_size, custom):  # input the point cloud for preprocessing
    # print(":: Downsample with a voxel size %.6f." % voxel_size)
    current_time = time.time()
    if custom == 'o3d':
        pcd_down = o3d.open3d.geometry.voxel_down_sample(pcd, voxel_size)  # down sample by o3d
    else:
        pcd_down = custom_down_sample(pcd, voxel_size)  # custom down sample function
    # print("down sample: ", time.time()-current_time)
    # under certain radius need to have at least nb_points, e.g. sphere contain 30 points
    ...
    radius_normal = voxel_size * 2  # kdtree parameter for estimate normal radius
    # print(":: Estimate normal with search radius %.6f." % radius_normal)

    # estimate normal, 30 neighbors in normal radius
    o3d.open3d.geometry.estimate_normals(pcd_down,
        o3d.geometry.KDTreeSearchParamHybrid(radius=radius_normal, max_nn=30))

    radius_feature = voxel_size * 5  # kdtree parameter for estimate feature radius
    # print(":: Compute FPFH feature with search radius %.6f." % radius_feature)
    # calculate point cloud feature for later use
...
def prepare_dataset(voxel_size, image_name, custom):
    ...
    source_down, source_fpfh = preprocess_point_cloud(source, voxel_size, custom)
...
def execute_global_registration(source_down, target_down, source_fpfh,
    ...
    distance_threshold = voxel_size * 2.0  # 设定距离阈值为体素的1.5倍
...
def o3d_img_to_3d(total_img, width=512):  # start=0, total_img=11
    focal = float(width / (np.tan(np.pi / 4) * 2))  # for virtual camera, 1 pixel = 1 mm. width=512mm, focal = 256
    in_matrix = o3d.camera.PinholeCameraIntrinsic(width, width, focal, focal, 256, 256)  # intrinsic matrix as explain
    progress = tqdm(total=len(total_img), desc="o3d_img_to_3d")   # progress bar, -start
    # os.makedirs('./save/RGBD/', exist_ok=True)
    for image_name in total_img:  # range(start, total_img):
        pinhole_camera_intrinsic = o3d.camera.PinholeCameraIntrinsic(width, width, focal, focal, width / 2.0,
                                                                     width / 2.0)
        source_color = o3d.io.read_image("./save/test/2nd_gt/{}.png".format(image_name))  # generate/
        # ./save/generate/images/, ./save/test/apartment0-37-hw1/pred/, 1st_sem, 1st_sem_gt, semantic, 2nd
        # source_color = o3d.io.read_image("./save/semantic/{}.png".format(image_name))
        source_depth = o3d.io.read_image("./save/test/2nd_depth/{}.png".format(image_name))  # generate/
        # ./save/generate/depth/, ./save/test/apartment0-37-hw1/depth/
        source_rgbd_image = o3d.open3d.geometry.create_rgbd_image_from_color_and_depth(
            source_color, source_depth, depth_scale=width / 2.0, convert_rgb_to_intensity=False)
        source = o3d.geometry.create_point_cloud_from_rgbd_image(source_rgbd_image, pinhole_camera_intrinsic)
...
def reconstruction(num_of_img, voxel_size=0.00001, mode='o3d', custom='o3d'):
    # voxel_size = 0.00001
    trans_list = []
    vis = o3d.visualization.Visualizer()
    os.makedirs('./save/pcd/', exist_ok=True)
    target, target_down, target_fpfh = prepare_dataset(voxel_size, image_name=num_of_img[0], custom=custom)
    ...
    progress = tqdm(total=len(num_of_img))
    for idx, img in enumerate(num_of_img, 1):
        source, source_down, source_fpfh = prepare_dataset(voxel_size, image_name=img, custom=custom)
        ...
        # track_gt_arr[idx] = np.dot(ex_matrix, track_gt_arr[idx].T).T
        ...
        # progress.set_description("{}".format(str(result_ransac)[38:119]))
        # progress.set_description("{}".format(result_ransac))
        progress.update(1)
...
if __name__ == "__main__":
    num_of_img_start = 0
    # num_of_img = len(os.listdir('./save/generate/RGB/'))-1  # generate/
    path = './save/test/2nd_gt/'
    # ./save/generate/images/, ./save/test/apartment0-37-hw1/pred/, habitat-52-hw1, 1st_gt, semantic, 1st_sem, 2nd
    num_of_img = [os.path.splitext(filename)[0] for filename in os.listdir(path)]
    num_of_img.sort()
    # num_of_img = num_of_img[:10]
    print(num_of_img)
    # num_of_img = 10
    mode = 'o3d'  # only operate o3d function, otherwise implement my own function.
    custom = 'o3d'  # custom down sampling if not o3d
    voxel_size = 1e-5  # gt: 1e-5
    print("voxel size: ", voxel_size)
    if mode == 'o3d':
        o3d_img_to_3d(total_img=num_of_img)
        print('open3d function')
    else:
        img_to_3d(start=num_of_img_start, total_img=num_of_img+1)
        print('my own implementation')
    if custom == 'o3d':
        print('open3d down sampling')
    else:
        print('custom down sampling')
    reconstruction(num_of_img=num_of_img, voxel_size=voxel_size, mode=mode, custom=custom)
