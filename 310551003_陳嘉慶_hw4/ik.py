...
def your_ik(robot, new_pose: list or tuple or np.ndarray,  # new_pose: end effector pose
            max_iters: int = 3000, stop_thresh: float = .001):
    # you may use this params to avoid joint limit,
    joint_limits = np.asarray([  # for checking output value, prevent exceed value
    ...
    q_states = p.getJointStates(robot.robot_id, range(0, num_q))  # q: joint poses
    ...
    for i in range(0, max_iters):
        ...
        # inv_jacobian = np.dot(current_jacobian.T, np.linalg.inv(np.dot(current_jacobian, current_jacobian.T)))
        inv_jacobian = current_jacobian.T
        # print("T_jacobian: ", inv_jacobian)
        ...
        if err > 1:  # all test
            step_size = 0.1
        # elif i > 1000:  # single test for hard
        #     step_size = 0.0005
        # elif i > 2000:  # single test for hard
        #     step_size = 0.0001
        else:  # all test
            step_size = 0.12  # inv: 0.012, transpose: 0.12
...
def score_ik(robot, testcase_files: str, visualize: bool = False):
    ...
    for file_id, testcase_file in enumerate(testcase_files):
        ...
        for i in range(cases_num):
            ...
            gt_joint_poses = pybullet_ik(robot, poses[i])
            # print("your_joint_poses: {}, \ngt_joint_poses: {} ".format(your_joint_poses, gt_joint_poses))
